...
// Base URL for API endpoints - will be dynamically set based on environment
const getBaseUrl = () => {
  // If we're in production (Zeabur), use the production domain
  if (window.location.hostname.includes('zeabur.app')) {
    return 'https://misceltutorai-demo.zeabur.app';
  }
  // Otherwise use localhost for development
  return 'http://localhost:3001';
};

// Use direct endpoint (working reliably)
const DIRECT_API_URL = `${getBaseUrl()}/api/chat-direct`;
// Fallback proxy endpoint (if direct fails)
...
async function callAI(userMessage, context = {}, controller = null) {
    ...
    // Try direct endpoint first (working reliably), then fallback to proxy
    let response;
    let attempts = 0;
    const maxAttempts = 2;
    ...
    while (attempts < maxAttempts) {
        try {
            const url = attempts === 0 ? DIRECT_API_URL : API_URL;
            const headers = attempts === 0 ? {
            ...
            const body = attempts === 0 ? JSON.stringify({
                messages: messages
            }) : JSON.stringify({
                model: 'deepseek-chat',
                messages: messages,
            ...
            response = await currentAIRequest;

            if (!response.ok) {
                const errorText = await response.text();
                console.error(`API Error (${url}): ${response.status} - ${errorText}`);
                
                // If direct fails, try proxy endpoint
                if (attempts === 0) {
                    attempts++;
                    continue;
                } else {
            ...
            if (attempts === 0 && (error.message.includes('ECONNRESET') || error.message.includes('Failed to fetch'))) {
                attempts++;
                continue;
            } else {
                throw error;
